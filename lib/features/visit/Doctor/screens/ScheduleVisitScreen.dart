...
class _ScheduleVisitScreenState extends State<ScheduleVisitScreen> with SingleTickerProviderStateMixin {
  final _formKey = GlobalKey<FormState>();

  // Controllers
  final TextEditingController _dateController = TextEditingController();
  final TextEditingController _notesController = TextEditingController();
  final DoctorListController _doctorListController = Get.find<DoctorListController>();
            ...
            child: Column(
              children: [
                ...
                Padding(
                  ...
                  child: Text("Select Doctor", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.grey[800])),
                ),
                Padding(
                ...
                Expanded(
                  child: Obx(() {
                    var list = _doctorListController.filteredDoctors;
                    ...
                    return ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          title: Text(doctor.name, style: TextStyle(fontWeight: FontWeight.w600, color: isSelected ? TColors.primary : Colors.black87)),
                          subtitle: Text("ID: ${doctor.id.length > 8 ? doctor.id.substring(0, 8) : doctor.id}...", style: TextStyle(color: Colors.grey[500], fontSize: 12)),
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      if (_selectedDoctorId == null) {
        Get.snackbar("Required", "Please select a doctor",
            backgroundColor: Colors.orange.withOpacity(0.1), colorText: Colors.redAccent);
        return;
      }
      ...
      try {
        await DoctorVisitController.createDoctorVisit(
      ...
      appBar: AppBar(
        ...
        title: LayoutBuilder(builder: (context, constraints) {
          return MediaQuery.of(context).size.width < 600
              ? const Text("New Appointment", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600))
  ...
  Widget _buildMobileLayout(BoxConstraints constraints) {
    ...
    // Calculate a safe header height to prevent overlapping
    ...
    return SizedBox(
      ...
      child: Stack(
        children: [
          ...
          Positioned(
            top: headerHeight - 40, // Overlap by 40px, adjusted to not cover text
          ...
          SafeArea(
            child: Padding(
              ...
              child: Column(
                ...
                mainAxisAlignment: isMobile ? MainAxisAlignment.center : MainAxisAlignment.center, // Center vertically
                children: [
                  if(isMobile) const Spacer(), // Push content down a bit on mobile

                  if (!isMobile) ...[
                    const Icon(Icons.medical_services_outlined, size: 60, color: Colors.white24),
                    const SizedBox(height: 32),
                  ],
                  ...
                  if(isMobile) const SizedBox(height: 60), // Extra bottom padding on mobile banner to avoid overlap with form
            ...
            child: _buildInputCard(
              ...
              child: Row(
                children: [
                  Expanded(
                    child: Text(
                      _selectedDoctorName ?? "Tap to search doctor...",
                      style: TextStyle(
                        fontSize: 16,
                        fontWeight: _selectedDoctorName != null ? FontWeight.w600 : FontWeight.normal,
                        color: _selectedDoctorName != null ? Colors.black87 : Colors.grey[500],
                      ),
              ...
              child: Row(
                children: [
                  ...
                  Container(
                    ...
                    child: const Icon(Icons.edit_calendar_rounded, color: TColors.primary, size: 20),
                  )
                ],
              ),
            ),
        ...
        _AnimatedEntry(
          ...
          child: _buildInputCard(
            ...
            child: TextFormField(
              ...
              decoration: InputDecoration(
                hintText: "Write down the purpose of the visit...",
                hintStyle: TextStyle(color: Colors.grey[400], fontSize: 14),
                border: InputBorder.none,
                contentPadding: EdgeInsets.zero,
              ),
              validator: (value) => value!.isEmpty ? 'Notes are required' : null,
            ),
          ),
        ),
      ],
  ...
  Widget build(BuildContext context) {
    return SlideTransition(
      position: Tween<Offset>(begin: const Offset(0, 0.2), end: Offset.zero).animate(
        CurvedAnimation(parent: controller, curve: Interval(index * 0.1, 1.0, curve: Curves.easeOutCubic)),
      ),
      child: FadeTransition(
        opacity: Tween<double>(begin: 0, end: 1).animate(
          CurvedAnimation(parent: controller, curve: Interval(index * 0.1, 1.0, curve: Curves.easeOut)),
        ),
        child: child,
      ),
    );
