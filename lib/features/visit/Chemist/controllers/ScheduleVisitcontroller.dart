...
class StockistVisitController {
  ...
  static Future<void> createDoctorVisit({
    ...
    try {
      ...
      print(
          "ChemistVisitController: Response status code: ${response.statusCode}");
      ...
      if (response.statusCode == 201) {
        ...
        if (context.mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text(TTexts.uiTextChemistScheduleSuccessfullyCreated)),
          );
        }
      } else {
        print(
            "ChemistVisitController: Failed to create doctor visit, response not 201");
        if (context.mounted) {
          // Check if context is still mounted
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
